...
- **FR-112**: JOIN_REQUEST wire payload MUST contain: protocol_version (0x01), device name, MAC (6 bytes), X25519 pubkey (32 bytes), random nonce (8 bytes), Unix timestamp (4 bytes). Serialized as the binary format from the design doc (max 84 bytes), base64-encoded. The OMI InfoItem value is prefixed with the display_mode: `"color:<base64>"` or `"digit:<base64>"`. The display_mode is NOT part of the wire protocol — it is carried in the InfoItem value prefix so the gateway can report it to the owner.
