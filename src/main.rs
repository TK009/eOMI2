...
fn main() -> Result<()> {
    ...
    // Resolve API token: empty placeholder allows captive portal to start on
    // unconfigured devices (check_bearer_auth rejects all requests when empty).
    let api_token: &'static str = API_TOKEN.unwrap_or("");
