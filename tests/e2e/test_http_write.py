...
from helpers import omi_read, omi_write, omi_write_batch, omi_write_tree
...
def test_write_new_item(base_url, token):
    ...
    read = omi_read(base_url, "/Test/Value", token=token)
...
def test_write_string_value(base_url, token):
    ...
    read = omi_read(base_url, "/Test/Str", token=token)
    assert read["response"]["status"] == 200
    values = read["response"]["result"]["values"]
...
def test_write_bool_value(base_url, token):
    ...
    read = omi_read(base_url, "/Test/Bool", token=token)
    assert read["response"]["status"] == 200
    values = read["response"]["result"]["values"]
...
def test_write_batch(base_url, token):
    ...
    # Read each back
    for item in items:
        read = omi_read(base_url, item["path"], token=token)
        assert read["response"]["status"] == 200
        values = read["response"]["result"]["values"]
...
def test_write_tree_merge(base_url, token):
    ...
    read = omi_read(base_url, "/TestTree/Sub/Leaf", token=token)
    assert read["response"]["status"] == 200
    values = read["response"]["result"]["values"]
